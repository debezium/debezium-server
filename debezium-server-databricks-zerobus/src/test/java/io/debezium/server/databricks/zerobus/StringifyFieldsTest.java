...
/**
 * Unit tests for the {@link StringifyFields} SMT, which serializes selected struct/array fields to a
 * JSON string so they can feed a Databricks Zerobus {@code VARIANT} column while sibling scalar
 * fields stay strongly typed.
 */
class StringifyFieldsTest {
    ...
    void serializesStructFieldToJsonStringAndRetypesToString() {
        ...
        // target field is now a STRING holding the JSON, siblings keep their types
    ...
    void acceptsCommaSeparatedStringConfig() {
        // Debezium passes SMT config values as raw strings (not parsed through ConfigDef).
        smt.configure(Map.of(StringifyFields.FIELDS_CONFIG, "props"));
        Schema schema = valueSchemaWithProps();
    ...
    void leavesUntargetedFieldsUntouched() {
        ...
        SourceRecord out = smt.apply(recordWith(schema, value));
        // untargeted fields keep original schema types
    ...
    void passesThroughWhenTargetFieldIsNull() {
        ...
        Schema schema = SchemaBuilder.struct().name("Value")
                ...
                .build();
        Struct value = new Struct(schema).put("id", 1); // props left null

        SourceRecord out = smt.apply(recordWith(schema, value));
        // schema still retyped to (optional) STRING, value stays null
        assertThat(out.valueSchema().field("props").schema().type()).isEqualTo(Schema.Type.STRING);
